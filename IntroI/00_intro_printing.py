
# This is a comment

# lets print a string
# console.log("Hello, CSPT13!", "Some other text", "and theres more...")
# print("Hello, CSPT13!", "Some other text", "and theres more...")
# print("Hello, CSPT13!", "Some other text", "and theres more...")
# print("Hello")
# variables
# label = value
# let const var (js)
# int bool short (c)



first_name = "Tom"
# print("Hello CSPT13 and " + first_name)
# num = 23.87

# # f strings
# print(f"this is a number: ({5+5})") # I can add non numbers here too
# my_string = "    this is a string tom    "
# print(my_string)

# # print(my_string.strip()) # takes away whitespaces on both sides
# print(len(my_string))
# print(len(my_string.strip())) # loses 8 character


print(f"Hello CSPT13 and           {len('this is a test')}     {first_name}.......".strip())
# print("something on a new line")

# first_name